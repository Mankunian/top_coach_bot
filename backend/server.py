    ...
    def do_POST(self):
        try:
            ...
            if self.path == '/api/me':
                ...
                user.update(fullName=name.strip(), bio=bio.strip())
                with connect() as db:
                    save_user(db, user)
                return self.result(200, user)
            return self.result(404, {'error': 'Not found'})
        except (ValueError, KeyError, TypeError):
            return self.result(400, {'error': 'Invalid request'})
        except Exception:
...
if __name__ == '__main__':
    if os.environ.get('RAILWAY_ENVIRONMENT') and not os.environ.get('DB_PATH', '').startswith('/data/'):
