...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if route.path == '/health':
            with connect() as db:
                db.execute('SELECT 1')
            return self.result(200, {'status': 'ok', 'version':'crm-1', 'database':'postgresql' if db.pg else 'sqlite'})
    ...
    def do_POST(self):
        try:
            ...
            if self.path == '/api/me':
                ...
                user.update(fullName=name.strip(), bio=bio.strip(),cityId=city['id'],cityCode=city['code'],city=city['name'],venue=str(data.get('venue',user.get('venue','')))[:250],address=str(data.get('address',user.get('address','')))[:250])
                with connect() as db:
                    save_user(db, user)
