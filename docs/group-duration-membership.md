...
Существующая JSON-модель CRM сохранена. Group.durationMinutes: integer minutes 1-1440, default 60. Session.durationMinutes — снимок длительности при генерации; ends = begins + durationMinutes * 60.
...
Billing uses actual session minutes (ends - begins). Calculations round the compatible hour balance to whole minutes before subtracting. Absent students are not charged; insufficient balances are capped at zero. Existing completed charges remain unchanged. The one-minute minimum is temporary for testing; a 60-minute minimum is deferred.
